...
PROCEDURE InoRepos()
   ...
   LOCAL aTin := { NtChr( LEFT( DTOS( DATE() ),6 ),"F" ),DATE() }
DEFINE DIALOG oDlg TITLE "Pasa Reposiciones a Diskette" FROM 1, 2 TO 10,44
   ...
   @ 02,82 GET oGet[1] VAR aTin[1] OF oDlg SIZE 40,12 PIXEL
   @ 16,00 SAY "Fecha   Final [DD.MM.AA]" OF oDlg RIGHT PIXEL SIZE 80,10
   @ 16,82 GET oGet[2] VAR aTin[2] OF oDlg ;
      VALID aTin[2] >= aTin[1] SIZE 40,12 PIXEL
   @ 32,40 BUTTON oGet[3] PROMPT "&Aceptar"  SIZE 44,12 OF oDlg ;
      ACTION ( MsgRun( "Copiando CODIGOS","Por favor Espere",;
               { |oDlg| CopioRep(aTin,oDlg) } ), oDlg:End() ) PIXEL
...
//------------------------------------//
STATIC PROCEDURE CopioRep( aVt,oDlg )
...
cQry := "SELECT c.optica, c.fecingre, c.ingreso, c.codigo_nit, d.pcosto, "+;
        ...
        "WHERE d.indica    = 'A'"                    +;
         ...
         " AND c.fecingre >= " + xValToChar( aVt[1] )+;
         " AND c.fecingre <= " + xValToChar( aVt[2] )+;
         " AND c.cgeconse  > 0"                      +;
...
cQry := "SELECT c.optica, c.fecharep, c.numrep, c.numfac, d.codigo, d.pcosto" +;
        ...
        "WHERE d.indica   <> 'E'"                    +;
         ...
         " AND c.fecharep >= " + xValToChar( aVt[1] )+;
         " AND c.fecharep <= " + xValToChar( aVt[2] )+;
         " ORDER BY c.numrep"
hRes := If( MSQuery( oApl:oMySql:hConnect,cQry ) ,;
            MSStoreResult( oApl:oMySql:hConnect ), 0 )
nL   := MSNumRows( hRes )
...
cQry := "SELECT d.destino, d.fechad, d.documen, c.consedev, d.codigo, d.pcosto, d.cantidad, "+;
        ...
        "WHERE d.indica <> 'E'"                    +;
         ...
         " AND c.fechad >= " + xValToChar( aVt[1] )+;
         " AND c.fechad <= " + xValToChar( aVt[2] )+;
...
cQry := "SELECT optica, fecha, numero, codigo, pcosto, "+;
        ...
        "FROM cadajust "                         +;
        "WHERE indica <> 'E'"                    +;
         " AND fecha  >= " + xValToChar( aVt[1] )+;
         " AND fecha  <= " + xValToChar( aVt[2] )+;
...
Tem->(dbCloseArea())
If hRes > 0
// ExportDBF( oApl:oNit,{ "Exportar","P" } )
// aRes := oApl:cRuta2 + "CADCLIEN.DBF"
   aRes := ""
   AEVAL( oApl:aWHija, { | xV,nP | aRes += ArrayValor( oApl:aOptic,STR(xV,2) ) + ", " } )
   cQry := oApl:cRuta2 + "INVTEM.DBF"
   cCod := 1
   MsgGet( aRes,"Cantidad de COPIAS",@cCod )
   cFile:= CreaCod() + "INVTEM.DBF"
   FOR nL := 1 TO cCod
      If MsgYesNo( "Inserte en la Unidad DISKETTE #"+STR(nL,2),">>> Por Favor <<<" )
         COPY FILE &(cQry) TO &(cFile)
//       COPY FILE &(cQry) TO A:Invtem.dbf
//       COPY FILE &(aRes) TO A:CADCLIEN.dbf
      EndIf
   NEXT nL
EndIf
oApl:aWHija := { NIL,NIL }
...
STATIC PROCEDURE GrabaTem( cCodigo,nMov,nCon,nOptica,cFactu,aR )
   LOCAL nMarca
...
If nOptica # 0 .AND. nOptica # 4
   If ASCAN( oApl:aWHija, nOptica ) == 0
       AADD( oApl:aWHija, nOptica )
   EndIf
EndIf
...
PROCEDURE InoRepOp()
   LOCAL oDlg, oGet := ARRAY(3), aTin
   LOCAL cRut := CreaCod()
If LEFT( cRut,2 ) == "A:"
   MsgInfo( "Inserte el Diskette con las Reposiciones","POR FAVOR" )
EndIf
If !AbreDbf( "Tem","INVTEM",,cRut )
   RETURN
EndIf
aTin := { 0,CTOD(""),"",4,"FECREPOS" }
...
//------------------------------------//
STATIC PROCEDURE ActuRepos( aTin,oDlg,cRut )
   LOCAL aRep, cOri, oTB := ARRAY(9), cTemp := oApl:cRuta2 + "INVTEM.DBF"
cOri := cRut + "INVTEM.DBF"
...
While !Tem->(EOF())
   aRep[3] += Tem->CANTIDAD
   aRep[4] ++
   aTin[3] := Tem->CODIGO + STR(aRep[4])
   oDlg:Update()
   oApl:nEmpresa := Tem->OPTICA
   CreaCod( Tem->CODIGO )
   do Case
   Case Tem->MOV == 1 .AND. (Tem->OPTICA == oApl:oEmp:OPTICA .OR. aRep[6])
      aRep[1] := MAX( Tem->NUMREPOS,aRep[1] )
      Reposicion( oTB,Tem->INDICA )
   Case Tem->MOV == 2 .AND. oApl:oEmp:ENLINEA
      aRep[2] := MAX( Tem->NUMREPOS,aRep[2] )
      CompraArt( oTB,Tem->INDICA )
   Case Tem->MOV == 3 .AND. (Tem->OPTICA == oApl:oEmp:OPTICA .OR. aRep[6])
      aRep[7] := MAX( Tem->CONSEDEV,aRep[7] )
      Devolucion( oTB,Tem->INDICA )
   Case Tem->MOV == 4 .AND. oApl:oEmp:ENLINEA
      aRep[2] := MAX( Tem->NUMREPOS,aRep[2] )
      CompraMon( oTB,aRep )
   Case Tem->MOV == 5 .AND. (Tem->OPTICA == oApl:oEmp:OPTICA .OR. aRep[6])
...
//------------------------------------//
STATIC PROCEDURE CambiaMon( cCodi,nCanti,nMov,lCambio,nPC )
...
//-----------------2------------------//
STATIC PROCEDURE CompraArt( oTB,cIndica )
...
//-----------------4------------------//
STATIC PROCEDURE CompraMon( oTB,aRep )
...
//-----------------3------------------//
STATIC PROCEDURE Devolucion( oTB,cIndica )
   LOCAL cQry
...
//------------------------------------//
STATIC PROCEDURE DevolM( oTB,cCodi,nOpt,nCanti )
   LOCAL aDV, nCDev
...
//-----------------1------------------//
STATIC PROCEDURE Reposicion( oTB,cIndica )
...
//-----------------5------------------//
STATIC PROCEDURE Ajustes( oTB,cIndica,nMov )
